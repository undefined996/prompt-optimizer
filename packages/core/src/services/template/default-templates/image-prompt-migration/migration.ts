...
7. 变量也必须在这一次视觉调用里一并完成，不要只给 prompt 不给 defaults。
8. 只要当前提示词里存在可复用的主体、数量、颜色、关键动作、场景主题等核心信息，就优先提取 2 到 5 个高复用变量；只有确实没有合适变量时，defaults 才输出 {}。
9. 每个 defaults 键都必须已经以 {{变量名}} 的形式出现在 prompt 中，禁止使用 {variable} 或其他占位形式；不要把大量风格词做成变量，也不要给 prompt 中没有出现的变量补 defaults。
