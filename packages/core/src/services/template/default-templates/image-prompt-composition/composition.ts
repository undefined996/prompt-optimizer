...
6. variables 不是重点。只有在确实有高复用价值时才提取，最多 5 个。
7. 不要为了模板感强行塞入变量；如果没有合适变量，defaults 输出 {}。
8. 如果提取变量，每个 defaults 键都必须已经以 {{变量名}} 的形式出现在 prompt 中，禁止使用 {variable} 或其他占位形式。
9. 如果提取变量，优先放在主体、数量、颜色、关键对象、关键动作等信息上，不要把大量风格词变量化。
