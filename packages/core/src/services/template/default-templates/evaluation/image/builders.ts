...
Please analyze the current workspace ${subjectLabel} and return a strict JSON assessment.`;
  }
...
请分析当前工作区${subjectLabel}，并返回严格的 JSON 评估结果。`;
};
...
export const createImageAnalysisTemplate = (
  ...
  content: [
    {
      role: 'system',
      content: buildImageAnalysisSystemPrompt(identity.language, config),
    },
    {
      role: 'user',
      content: buildImageAnalysisUserPrompt(identity.language, config),
    },
  ] as MessageTemplate[],
