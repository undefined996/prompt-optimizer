  ...
  evaluation: {
    ...
    type: {
      ...
      compareConfig: {
        ...
        dialogTitle: "Compare Role Setup",
        helper: "Assign compare roles to test slots so structured compare can understand the target, previous version, and learnable references.",
        requireTarget: "Multiple workspace slots were detected. Please choose which slot should act as the target.",
        targetRequired: "Select at least one workspace slot as the target.",
        targetNeededShort: "Target required",
        reviewNeededShort: "Review needed",
        autoDetected: "Auto",
        manualAssigned: "Manual",
        expiredManualTag: "Expired manual role",
        expiredManualSummary: "{count} saved manual role assignments expired and fell back to auto-detection.",
        workspaceChangedTag: "Needs review",
        workspaceChangedSummary: "{count} saved workspace role assignments need review because the workspace prompt changed.",
        keepAuto: "Keep auto-detection",
        keepAutoWithRole: "Keep auto-detection (current: {role})",
        useInferred: "Use Auto Detection",
        clearManual: "Clear Manual Roles",
        previewModeLabel: "Mode Preview",
        previewPairsLabel: "Executable Pairwise",
        previewReasonsLabel: "Current Notes",
        previewModeStructured: "Structured Compare",
        previewModeGeneric: "Generic Compare",
        genericFallbackSummary: "This configuration will not enter structured compare and will fall back to generic compare.",
        confirmDisabled: "Resolve conflicts before confirming",
        noVersionLabel: "No version",
        noModel: "No model",
        pairValues: {
          targetBaseline: "Target vs Previous",
          targetReference: "Target vs Reference",
          referenceBaseline: "Reference vs Reference Baseline",
          targetReplica: "Target vs Replica",
        },
        reasonValues: {
          duplicateTarget: "Multiple targets are assigned, so structured compare cannot identify one optimization focus.",
          duplicateBaseline: "Multiple baselines are assigned, so structured compare cannot identify one previous version.",
          duplicateReference: "Multiple references are assigned, so structured compare cannot identify one learnable reference.",
          duplicateReferenceBaseline: "Multiple reference baselines are assigned, so structured compare cannot identify one reference baseline.",
          hasAuxiliarySnapshot: "Some selected tests cannot participate in meaningful pairwise judging, so compare falls back to holistic multi-test comparison.",
          missingTarget: "There is no usable target, so structured compare cannot start.",
          missingStructuredCompanion: "The current target lacks a baseline, reference, or replica pair for core pairwise judging.",
          referenceBaselineWithoutReference: "A reference baseline exists without a reference, so that evidence will not participate in structured judging.",
        },
        roleValues: {
          target: "Target",
          baseline: "Previous",
      ...
      compareMetadata: {
        ...
        decision: {
          ...
          headlines: {
            continue: "The target is moving in the right direction, but there is still actionable improvement headroom.",
            ...
            regressed: "The target appears to have regressed relative to the previous version; do not accept this rewrite directly.",
          },
          actions: {
            ...
            reviewPromptValidity: "Check whether the prompt change is actually transferable, because the reference-side evidence does not currently support it.",
            learnFromReference: "Learn from the stronger reference-side structure before the next rewrite.",
      ...
      judgements: "Pairwise Judgements",
      pairHighlights: "Pair Highlights",
      progressSummary: "Progress Summary",
      referenceGapSummary: "Reference Gap",
      ...
      conflictSignals: "Conflict Checks",
      targetVsBaseline: "Target vs Previous",
      targetVsReferenceGap: "Target vs Reference Gap",
      improvementHeadroom: "Improvement Headroom",
      overfitRisk: "Overfit Risk",
      stopRecommendation: "Stop Recommendation",
      ...
      conflictSignalValues: {
        improvementNotSupportedOnReference: "The target improved over baseline, but the same prompt change is not supported on the reference side.",
        improvementUnstableAcrossReplicas: "The target improved in one comparison, but replica evidence suggests the gain may be unstable.",
        regressionOutweighsCosmeticGains: "Regression against the baseline should outweigh cosmetic improvements elsewhere.",
        sampleOverfitRiskVisible: "When reusable gains and sample-fitting gains coexist, prefer conservative conclusions and keep the overfit risk visible.",
      },
      signalValues: {
        targetVsBaseline: {
          improved: "Improved",
