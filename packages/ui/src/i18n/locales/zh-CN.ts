...
export default {
  ...
  evaluation: {
    ...
    compareConfig: {
      button: "对比角色",
      dialogTitle: "对比角色配置",
      helper: "为测试槽位指定对比角色，帮助结构化对比识别目标、上一版本和可学习参考。",
      requireTarget: "检测到多个 workspace 槽位，请明确指定哪个槽位是目标（target）。",
      targetRequired: "请至少为一个 workspace 槽位指定目标（target）。",
      targetNeededShort: "需指定目标",
      reviewNeededShort: "待复核",
      autoDetected: "自动识别",
      manualAssigned: "手动指定",
      expiredManualTag: "旧手动配置已失效",
      expiredManualSummary: "检测到 {count} 条旧的手动角色配置已失效，当前已回退到自动识别结果。",
      workspaceChangedTag: "待复核",
      workspaceChangedSummary: "检测到 {count} 条工作区手动角色对应的 prompt 已变更，请重新确认这些角色。",
      keepAuto: "保持自动识别",
      keepAutoWithRole: "保持自动识别（当前：{role}）",
      useInferred: "使用自动识别",
      clearManual: "清除手动设置",
      previewModeLabel: "模式预览",
      previewPairsLabel: "可执行 Pairwise",
      previewReasonsLabel: "当前说明",
      previewModeStructured: "结构化对比",
      previewModeGeneric: "通用对比",
      genericFallbackSummary: "当前配置不会进入 structured compare，将回退为通用 compare。",
      confirmDisabled: "存在冲突，无法确认",
      noVersionLabel: "未指定版本",
      noModel: "未指定模型",
      pairValues: {
        targetBaseline: "目标 vs 上一版本",
        targetReference: "目标 vs 参考",
        referenceBaseline: "参考 vs 参考基线",
        targetReplica: "目标 vs 副本",
      },
      reasonValues: {
        duplicateTarget: "存在多个 target，structured compare 无法判断唯一优化目标。",
        duplicateBaseline: "存在多个 baseline，structured compare 无法判断唯一上一版本。",
        duplicateReference: "存在多个 reference，structured compare 无法判断唯一学习参考。",
        duplicateReferenceBaseline: "存在多个 referenceBaseline，structured compare 无法判断唯一参考基线。",
        hasAuxiliarySnapshot: "当前选中的部分测试无法形成有意义的 pairwise judge，因此会整体退回多测试综合对比。",
        missingTarget: "当前没有可用的 target，因此无法进入 structured compare。",
        missingStructuredCompanion: "当前 target 缺少 baseline / reference / replica 配对，无法执行核心 pairwise judge。",
        referenceBaselineWithoutReference: "存在 referenceBaseline，但没有 reference；这组证据当前不会参与 structured judge。",
      },
      roleValues: {
        target: "目标",
        baseline: "上一版本",
        reference: "参考",
        referenceBaseline: "参考基线",
        replica: "副本",
        auxiliary: "辅助",
      },
    },
      compareMetadata: {
        ...
        decision: {
          ...
          nextActions: "下一步建议",
          headlines: {
            continue: "当前目标方向是对的，但仍然存在可执行的改进空间。",
            stop: "当前结果已经接近收敛，继续自动改写的收益可能有限。",
            review: "当前对比结果建议先人工复核，再决定是否继续改写。",
            regressed: "当前目标相较上一版本出现退步，暂时不应直接接受这次改写。",
          },
          actions: {
            inspectRegression: "先检查新版本删掉了什么、削弱了什么，再决定是否继续改写。",
            reviewBeforeRewrite: "先复核证据冲突点，再决定是继续改写还是保留当前版本。",
            reviewPromptValidity: "先检查这次 prompt 改动是否真的可迁移，因为 reference 侧暂时并不支持它。",
            learnFromReference: "下一轮优先学习参考侧更强的结构策略，而不是做大范围改动。",
        ...
        focusSummaries: "聚合焦点结论",
        mode: "模式",
        roles: "快照角色",
      judgements: "成对判断",
      pairHighlights: "关键对比结论",
      progressSummary: "进步判断",
      referenceGapSummary: "参考差距",
      ...
      conflictSignals: "冲突检查",
      targetVsBaseline: "目标 vs 上一版本",
      targetVsReferenceGap: "目标 vs 参考差距",
      improvementHeadroom: "改进空间",
      overfitRisk: "过拟合风险",
      stopRecommendation: "停止建议",
      stopReasons: "停止原因",
      modeValues: {
        structured: "结构化",
        generic: "通用",
      ...
      conflictSignalValues: {
        improvementNotSupportedOnReference: "target 虽有进步，但同一改动没有得到 reference 侧支持。",
        improvementUnstableAcrossReplicas: "target 虽有进步，但 replica 证据提示收益并不稳定。",
        regressionOutweighsCosmeticGains: "相对 baseline 的退步应优先于其他表面优化。",
