  ...
  evaluation: {
    ...
    type: {
      ...
      compareConfig: {
        button: "對比角色",
        dialogTitle: "對比角色配置",
        helper: "為測試槽位指定對比角色，幫助結構化對比識別目標、上一版本與可學習參考。",
        requireTarget: "偵測到多個 workspace 槽位，請明確指定哪個槽位是目標（target）。",
        targetRequired: "請至少為一個 workspace 槽位指定目標（target）。",
        targetNeededShort: "需指定目標",
        reviewNeededShort: "待複核",
        autoDetected: "自動識別",
        manualAssigned: "手動指定",
        expiredManualTag: "舊手動配置已失效",
        expiredManualSummary: "偵測到 {count} 筆舊的手動角色配置已失效，當前已回退到自動識別結果。",
        workspaceChangedTag: "待複核",
        workspaceChangedSummary: "偵測到 {count} 筆工作區手動角色對應的 prompt 已變更，請重新確認這些角色。",
        keepAuto: "保持自動識別",
        keepAutoWithRole: "保持自動識別（目前：{role}）",
        useInferred: "使用自動識別",
        clearManual: "清除手動設定",
        previewModeLabel: "模式預覽",
        previewPairsLabel: "可執行 Pairwise",
        previewReasonsLabel: "目前說明",
        previewModeStructured: "結構化對比",
        previewModeGeneric: "通用對比",
        genericFallbackSummary: "目前配置不會進入 structured compare，將回退為通用 compare。",
        confirmDisabled: "存在衝突，無法確認",
        noVersionLabel: "未指定版本",
        noModel: "未指定模型",
        pairValues: {
          targetBaseline: "目標 vs 上一版本",
          targetReference: "目標 vs 參考",
          referenceBaseline: "參考 vs 參考基線",
          targetReplica: "目標 vs 副本",
        },
        reasonValues: {
          duplicateTarget: "存在多個 target，structured compare 無法判定唯一優化目標。",
          duplicateBaseline: "存在多個 baseline，structured compare 無法判定唯一上一版本。",
          duplicateReference: "存在多個 reference，structured compare 無法判定唯一學習參考。",
          duplicateReferenceBaseline: "存在多個 referenceBaseline，structured compare 無法判定唯一參考基線。",
          hasAuxiliarySnapshot: "目前選中的部分測試無法形成有意義的 pairwise judge，因此會整體退回多測試綜合對比。",
          missingTarget: "目前沒有可用的 target，因此無法進入 structured compare。",
          missingStructuredCompanion: "目前 target 缺少 baseline / reference / replica 配對，無法執行核心 pairwise judge。",
          referenceBaselineWithoutReference: "存在 referenceBaseline，但沒有 reference；這組證據目前不會參與 structured judge。",
        },
        roleValues: {
          target: "目標",
          baseline: "上一版本",
          reference: "參考",
        referenceBaseline: "參考基線",
        replica: "副本",
        auxiliary: "輔助",
      },
    },
      compareMetadata: {
        ...
        decision: {
          ...
          nextActions: "下一步建議",
          headlines: {
            continue: "目前目標方向是對的，但仍然存在可執行的改進空間。",
            stop: "目前結果已接近收斂，繼續自動改寫的收益可能有限。",
            review: "目前對比結果建議先人工複核，再決定是否繼續改寫。",
            regressed: "目前目標相較上一版本出現退步，暫時不應直接接受這次改寫。",
          },
          actions: {
            inspectRegression: "先檢查新版本刪掉了什麼、削弱了什麼，再決定是否繼續改寫。",
            reviewBeforeRewrite: "先複核證據衝突點，再決定是繼續改寫還是保留目前版本。",
            reviewPromptValidity: "先檢查這次 prompt 改動是否真的可遷移，因為 reference 側暫時並不支持它。",
            learnFromReference: "下一輪優先學習參考側更強的結構策略，而不是做大範圍改動。",
        ...
        focusSummaries: "聚合焦點結論",
        mode: "模式",
        roles: "快照角色",
      judgements: "成對判斷",
      pairHighlights: "關鍵對比結論",
      progressSummary: "進步判斷",
      referenceGapSummary: "參考差距",
      ...
      conflictSignals: "衝突檢查",
      targetVsBaseline: "目標 vs 上一版本",
      targetVsReferenceGap: "目標 vs 參考差距",
      improvementHeadroom: "改進空間",
      overfitRisk: "過擬合風險",
      stopRecommendation: "停止建議",
      stopReasons: "停止原因",
      modeValues: {
        structured: "結構化",
        generic: "通用",
      ...
      conflictSignalValues: {
        improvementNotSupportedOnReference: "target 雖有進步，但同一改動沒有得到 reference 側支持。",
        improvementUnstableAcrossReplicas: "target 雖有進步，但 replica 證據提示收益並不穩定。",
        regressionOutweighsCosmeticGains: "相對 baseline 的退步應優先於其他表面優化。",
