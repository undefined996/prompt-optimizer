...
describe('reference image workspace theme guards', () => {
  it('uses the rebuilt reference-image dialog information architecture and removes legacy markup', () => {
    ...
    expect(source).toMatch(/reference-dialog-source-picker/)
    expect(source).toMatch(/reference-dialog-thumbnail/)
    expect(source).toMatch(/@click="triggerReferenceImageUpload"/)
    expect(source).toMatch(/@keydown\.enter\.prevent="triggerReferenceImageUpload"/)
    expect(source).toMatch(/reference-dialog-section/)
    expect(source).toMatch(/reference-dialog-results-grid/)
    expect(source).toMatch(/reference-dialog-status/)
    expect(source).toMatch(/referenceDialog\.hasProcessingStage/)
    expect(source).toMatch(/currentProcessingStageLabelKey/)
    expect(source).toMatch(/processingStageOptions/)
    expect(source).toMatch(/setProcessingStage\('generating-preview'\)/)
    ...
    expect(source).toMatch(/reference-dialog-current-prompt[\s\S]*v-if="referenceDialog\.showModeSwitch"/)
    expect(source).toMatch(/data-testid="reference-image-prompt-input"[\s\S]*:placeholder="''"/)
    expect(source).not.toMatch(/imageWorkspace\.referenceImage\.sourceImage/)
    expect(source).not.toMatch(/sourceImageHint/)
    expect(source).not.toMatch(/changeImage/)
    expect(source).not.toMatch(/sourceImageEmptyTitle/)
    ...
    expect(source).not.toMatch(/recognizingImage/)
    expect(source).not.toMatch(/extractingVariables/)
